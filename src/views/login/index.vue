...
// 获取 site_no（从 URL 路径中获取，格式：/site_no/xxx）
const getSiteNo = () => {
...
// 执行登录的函数
const executeLogin = async (forceLogin: number | undefined = undefined) => {
  const site_no = getSiteNo();
  const { username, password } = ruleForm;
  const { isRemembered, loginDay } = useUserStoreHook();

  // 构建登录数据，auth_type 固定为 1（密码登录）
  // auth_type 为 1 时，需要对 auth_data 进行加密
  ...
  // 如果传入了 force_login，添加到登录数据中
  if (forceLogin !== undefined) {
    loginData.force_login = forceLogin;
  }

  return useUserStoreHook().loginByUsername(loginData);
};
